...
export function mountSuggestModal(config) {
  ...
  if (tierSelect) {
    tierSelect.innerHTML =
      ...
      config.tiers
        .map(
          (t) =>
            `<option value="${t.label}">${t.label} — ${t.description.slice(0, 50)}…</option>`,
        )
        .join("");
  }

  if (dimCheckboxes) {
    dimCheckboxes.innerHTML = config.dimensions
      .map(
        (d) => `
  ...
  document
    .getElementById("sug-download")
    ?.addEventListener("click", () => downloadJson());
}

function gatherFormData() {
  const f = (id) => document.getElementById(id)?.value || "";
  const dims = [...document.querySelectorAll("#sug-dims input:checked")].map(
    (cb) => cb.value,
  );
  const langs = f("sug-langs")
    .split(",")
    .map((s) => s.trim())
    .filter(Boolean);
...
function toJsonObj(d) {
  return {
    ...
    indicators: Object.fromEntries(
      d.dims.map((dim) => [dim, `measures ${dim} by ...`]),
    ),
    useCases: [],
    license: d.license,
    languages: d.langs,
...
function downloadJson() {
  ...
  if (
    ...
    alert(
      "Please fill in all required fields: name, description, website, tier, software type, dimensions, and license.",
    );
    return;
  }
