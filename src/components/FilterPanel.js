...
export function renderFilters(containerId, config, tools, filters, onChange) {
  ...
  const licenses = [
    ...new Set(tools.map((t) => t.license).filter(Boolean)),
  ].sort();
  ...
  container.innerHTML = [
    ...
    chipGroup(
      ...
      config.dimensions.map((d) => ({
        ...
        activeStyle: `color:${d.color};border-color:${d.color};background:${d.color}18`,
      })),
      filters,
      "dim-chips",
    ),
  ...
  container.querySelectorAll(".chip[data-key]").forEach((chip) => {
    chip.addEventListener("click", () => {
      ...
      if (chip.dataset.activeStyle) {
        chip.style.cssText = now
          ? chip.dataset.activeStyle
          : chip.dataset.baseStyle || "";
      }
...
export function syncFilterChips(filters) {
  document.querySelectorAll(".chip[data-key]").forEach((chip) => {
    ...
    if (!active && chip.dataset.activeStyle)
      chip.style.cssText = chip.dataset.baseStyle || "";
    if (active && chip.dataset.activeStyle)
      chip.style.cssText = chip.dataset.activeStyle;
  });
}

function chipGroup(title, key, items, filters, id = "") {
  const chips = items
    .map(({ val, cls = "", title: tip = "", style = "", activeStyle = "" }) => {
